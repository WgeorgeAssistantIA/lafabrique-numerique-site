        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {p.items.map((item, i) => (
            <Reveal key={item.ref} delay={i * 100} className="h-full">
              <article className="card-lift group relative border border-line flex flex-col h-full hover:border-cyan">
                {item.image ? (
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="font-display uppercase text-2xl mt-4">
                    {item.title}
                  </h3>
                  ...
                  {item.press && item.press.length > 0 ? (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {item.press.map((mention) => (
                        <a
                          key={mention.label}
                          href={mention.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="fig-label border border-amber/50 text-amber px-2 py-1 text-[0.65rem] hover:bg-amber/10 transition-colors"
                        >
                          {mention.label}
                        </a>
                      ))}
